...
> = async (req: ApiRequest<z.infer<typeof createNewUserSchema>>, { logger }) => {
    ...
    const [existingUserEmail] = await db.select().from(users).where(eq(users.email, data.email));
    ...
    const [existingUserClerkId] = await db.select().from(users).where(eq(users.clerk_user_id, data.clerk_user_id));
